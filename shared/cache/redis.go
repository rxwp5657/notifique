...
func GetEntpointKey(url *url.URL, userId *string) Key {
	...
	key := fmt.Sprintf("%s:%s", hash.GetMd5Hash(url.Path), path)
...
func GetEndpointKeyWithPrefix(path string, userId *string) Key {
	...
	key := fmt.Sprintf("%s:%s", hash.GetMd5Hash(path), path)

	return Key(fmt.Sprintf("%s:%s", endpointKey, key))
}

func (rc *Redis) Set(ctx context.Context, k Key, value string, ttl time.Duration) error {
...
func (rc *Redis) Get(ctx context.Context, k Key) (string, error, bool) {
...
func (rc *Redis) Del(ctx context.Context, k Key) error {
...
func (rc *Redis) DelWithPrefix(ctx context.Context, prefix Key) error {
	var cursor uint64
	var keys []string
...
func NewRedisCache(client CacheRedisApi) (*Redis, error) {
