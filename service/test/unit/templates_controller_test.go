...
func testCreateNotificationTemplate(t *testing.T, e *gin.Engine, mock di.MockedBackend) {
	...
	tests := []struct {
		...
		},
		{
			...
			modifyRequest: func(req dto.NotificationTemplateReq) dto.NotificationTemplateReq {
				duplicated := sdto.TemplateVariable{
					Name:     "duplicated",
					Type:     "STRING",
					Required: false,
				}
...
func testGetNotificationTemplates(t *testing.T, e *gin.Engine, mock di.MockedBackend) {
	...
	testPage := sdto.Page[dto.NotificationTemplateInfoResp]{
	...
	tests := []struct {
		...
		expectedStatus int
		expectedError  *string
		expectedResp   *sdto.Page[dto.NotificationTemplateInfoResp]
	}{{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError != nil {
				...
				resp := sdto.Page[dto.NotificationTemplateInfoResp]{}
				if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
					t.Fatal(err)
				}
...
func testGetNotificationTemplateDetails(t *testing.T, e *gin.Engine, mock di.MockedBackend) {
	...
	testDetails := sdto.NotificationTemplateDetails{
		Id:               uuid.NewString(),
		Name:             testReq.Name,
	...
	tests := []struct {
		...
		expectedStatus int
		expectedError  *string
		expectedResp   *sdto.NotificationTemplateDetails
	}{
		{
		...
		},
		{
			...
			setupMock: func() {
				mock.Registry.MockNotificationTemplateRegistry.
					EXPECT().
					GetTemplateDetails(gomock.Any(), gomock.Any()).
					Return(sdto.NotificationTemplateDetails{}, internal.EntityNotFound{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError != nil {
				...
				resp := sdto.NotificationTemplateDetails{}
				if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
					t.Fatal(err)
				}
