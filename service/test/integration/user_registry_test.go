...
func TestUsersRegistryPostgres(t *testing.T) {
	...
	testInsertUserNotifications(ctx, t, tester)
}
...
func testInsertUserNotifications(ctx context.Context, t *testing.T, ust UserRegistryTester) {

	testNotifications, err := testutils.MakeTestUserNotifications(5, "")
	notificationsToInsert := make([]sdto.UserNotificationReq, 0, len(testNotifications))
	userIds := make([]string, 0, len(testNotifications))

	for i, n := range testNotifications {
		userIds = append(userIds, strconv.Itoa(i))
		notificationsToInsert = append(notificationsToInsert, sdto.UserNotificationReq{
	...
	t.Run("Can insert a batch of user notifications", func(t *testing.T) {
		notifications, err := ust.CreateNotifications(ctx, notificationsToInsert)
		assert.Nil(t, err)

		insertedNotifications := make([]sdto.UserNotificationReq, 0, len(testNotifications))
		...
		for _, userId := range userIds {
			...
			for _, n := range page.Data {
				insertedNotifications = append(insertedNotifications, sdto.UserNotificationReq{
					UserId:   userId,
					Title:    n.Title,
					Topic:    n.Topic,
					Contents: n.Contents,
		...
		assert.ElementsMatch(t, notificationsToInsert, insertedNotifications)
		assert.Equal(t, len(notifications), len(insertedNotifications))
	})
}
